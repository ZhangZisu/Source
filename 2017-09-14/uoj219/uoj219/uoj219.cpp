...
int main() {
	po[0] = 1;
	for (register int i = 1; i < MAXN; i++)po[i] = (long long)po[i - 1] * BASE % MOD;
	scanf("%d", &t);
	while (t--) {
		scanf("%s", s + 1); len = strlen(s + 1); memset(u, 0, sizeof(u)); memset(v, 0, sizeof(v)); ans = 0;
		for (register int i = 1; i <= len; i++)Hash[i] = ((long long)Hash[i - 1] * BASE + s[i] - 'a') % MOD;
		for (register int i = 1; (i << 1) <= len; i++) {
			for (register int j = 1; j + i <= len; j += i) {
				register int l = 0, r = std::min(i, j);
				while (l != r) {
